...
template <typename T> constexpr bool isNoneType() {
  return std::is_same<T, NoneType>::value ||
         std::is_same<T, const NoneType>::value;
}

template <typename StateT, typename EventT, typename GuardT = NoneType,
...
struct Transition {
  ...
  constexpr Transition(const State &s, const State &t, const Event &e,
    ...
    if constexpr (HasGuard()) {
      static_assert(std::is_invocable<Guard>::value,
                    "Guard should be invocable");
      static_assert(
          std::is_same<typename std::invoke_result<Guard>::type, bool>::value,
          "Guard should return bool.");
    }
    if constexpr (HasAction()) {
      static_assert(std::is_invocable<Action>::value,
                    "Action should be invocable");
      static_assert(
